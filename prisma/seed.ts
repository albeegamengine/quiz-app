...
async function main() {
  ...
  try {
    ...
    console.log(
      '既存データのクリアをスキップしました（テーブルが存在しないか、データがない可能性があります）'
    );
  }
  ...
  const questions = [
    // 複数選択式の質問
    {
      ...
      correctAnswer: 'すべて正解',
      explanation:
        'JavaScript では var、let、const のすべてが変数宣言に使用できます。ES6以降では let と const の使用が推奨されています。',
    },
    {
      ...
      explanation:
        'React では render() メソッドを使用してコンポーネントをレンダリングします。',
    },
    {
      ...
      explanation:
        'CSS でフレックスボックスレイアウトを有効にするには display: flex を使用します。',
    },
    {
      ...
      explanation:
        'HTTP ステータスコード 404 は「Not Found」を意味し、要求されたリソースがサーバー上に見つからないことを示します。',
    },
    {
      ...
      explanation:
        'TypeScript の主な利点は静的型チェックにより、コンパイル時にエラーを検出できることです。',
    },
    {
      ...
      explanation:
        'Next.js の App Router では page.tsx（または page.js）がページコンポーネントとして認識されます。',
    },
    {
      ...
      explanation:
        'Git でコミットを作成するには git commit コマンドを使用します。',
    },
    ...
    },
    {
      ...
      explanation:
        'JavaScript の配列で要素数を取得するには length プロパティを使用します。',
    },
    {
    ...
    },
    {
      ...
      explanation:
        'React でコンポーネントの状態を管理するには useState フックを使用します。',
    },
    {
